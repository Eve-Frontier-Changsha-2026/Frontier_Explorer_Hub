...
export function FulfillRequestForm({ requestId, onCancel, onSuccess }: Props) {
  ...
  const handleSubmit = async () => {
    if (!account || !plaintext.trim()) return;
    setError(null);
    try {
  ...
  return (
    <div className="border border-eve-gold/30 p-3 bg-eve-panel">
      ...
      <button
        ...
        disabled={!account || fulfillRequest.isPending || !plaintext.trim()}
      ...
      >
        {fulfillRequest.isPending ? "Encrypting & Submitting..." : "🔒 SUBMIT INTEL (ENCRYPTED)"}
      </button>
